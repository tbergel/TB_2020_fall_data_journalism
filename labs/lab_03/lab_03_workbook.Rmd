...
# A: Prince George's county. Rate was 29.3 per 1000 people.
...
# A: (I switched the question to lowest case rate, because in the initial question above, that is what's asked for) The answer is Bath, Virginia, which had a rate of 0.9 per 1000 people.
...
# A: 2, Hancock, Georgia and Galax City, Virginia.
...
covid_county_jun_aug_pct_change_death_rate <- covid_county_monthly_death_rate%>%
...
covid_county_jun_aug_pct_change_death_rate
...
```{r}
covid_county_mar_aug_pct_change <- covid_county_monthly_case_rate%>%
  select(county, state, d_2020_03_30, d_2020_08_30) %>%
  filter(state == "Massachusetts") %>%
  mutate(mar_aug_pct_change = (d_2020_08_30 - d_2020_03_30)/d_2020_03_30) %>%
  arrange(desc(mar_aug_pct_change))

# This code displays the results of the code we created above in the markdown document

covid_county_mar_aug_pct_change

# Q: which Massachusetts county had the largest percentage change in cumulative case rate between March and August?
