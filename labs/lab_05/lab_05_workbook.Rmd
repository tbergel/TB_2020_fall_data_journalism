...
To finish the lab, you have two exercises to complete.
...
# A: Delaware (DC is a trick answer because it's not actually a state)
...
```{r}
deaths_per_counties_per_state <- covid_county %>%
  group_by(state, county) %>%
  summarise(total_deaths = sum(deaths)) %>%
  arrange(desc(total_deaths))

# Display below
deaths_per_counties_per_state 

#Q: Which county had the highest total deaths? Which state is it in? What were the total deaths?
#A:New York City, New York with 23689 total deaths.
```
## You're done
